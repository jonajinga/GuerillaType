export default [
  {
    ...
    notes: [
      ...
      "What a share link carries, exactly: in the address, where a server can see it, numbers (wpm, raw wpm, accuracy, consistency, duration, character counts), the mode, the word list, the keyboard layout, whether the run was a personal best, whether a challenge was cleared, the date, and a public content id saying which quote, book page, poem, idiom, parable, lesson, challenge or drill was typed. Never the text typed, never a custom text's title, never keystrokes. When the text is one of your own, the words ride after the '#' -- browsers never send that part to this site, so it is decoded in the browser of whoever opens the link, and it rides along with the link wherever it is sent. Anyone holding the link can read all of it, so a shared link is public, and /privacy/ now says so in its own section.",
