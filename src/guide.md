...
- Share links: numbers and a public content id in the address; a custom text's words, and a keystroke replay when a run has one, only after the `#`, which browsers never send to a server. Anyone with the link can read it.
