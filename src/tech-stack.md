...
- **Self-hosted typing fonts** — Fira Code, IBM Plex Mono, Iosevka and Cascadia Code are opt-in choices for the typing surface and ship from `/assets/fonts/` rather than any CDN. Weight 400 only, subset to latin + latin-ext, 82 KB across eight `woff2` files, all SIL Open Font License 1.1 with the licence texts alongside them. Picking one costs a single same-origin request. Because they are same-origin the service worker caches them on first use; the cross-origin Bunny faces are skipped by `sw.njk` and never can be. (That is a difference in the font layer only — the practice page as a whole is not offline-capable today, because the same service worker deliberately passes `/assets/js/*.js` straight through to the network.) Provenance and the exact `pyftsubset` command are in [`src/assets/fonts/README.md`](#).
