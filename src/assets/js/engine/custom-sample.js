/* The sample custom text.
   ...
   about what the page does: that a long text is split into segments,
   that you pick the one you want, and that it remembers where you
   stopped. So a real one ships with the app.
   ...
   The passage is the opening of Alice's Adventures in Wonderland
   (Lewis Carroll, 1865, public domain), asciified to the site rule that
   typeable content carries no smart punctuation. Long enough to be
   worth typing and to make the segment picker mean something. */

import { read, write, KEY_CUSTOM_SAMPLE } from "../storage.js";
import { listSaved, saveText } from "./custom-text.js";

export const SAMPLE_TITLE = "Alice's Adventures in Wonderland (sample)";
...
/* Seed the sample if this browser has no saved texts and the user has
   ...
   Only seeding into an EMPTY list matters: it keeps the sample out of
   the way of someone who already has their own texts, and it keeps it
   from appearing in the middle of a test that seeded its own fixture. */
export async function ensureSample() {
  if (sampleDismissed()) return null;
  if (listSaved().length) return null;
  try {
    return await saveText({ title: SAMPLE_TITLE, raw: SAMPLE_TEXT, sample: true });
  } catch {
